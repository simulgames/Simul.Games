...
<div class="select-none">
    <div class="mt-4 flex">
        {#each firstRow as key}
            <KeyButton {key} Class={KeyBoardClasses[key]} />
        {/each}
    </div>
    ...
    <div class="mt-1 flex">
        ...
        <KeyButton
            ...
            Class="flex-grow-[1.50] {isEnterDisabled
                ? 'enabled:text-white'
                : 'bg-blue-600 dark:enabled:bg-blue-700 enabled:hover:bg-blue-700 dark:enabled:hover:bg-blue-600 enabled:text-white'} " />
        <div class="flex-grow-[0.75]" />
    </div>
</div>
